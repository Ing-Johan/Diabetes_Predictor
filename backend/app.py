...
columnas       = cargar('columnas.pkl')  
label_encoder  = cargar('label_encoder.pkl') 
...
def decodificar(pred_num):
    """Convierte 0/1 → 'No Diabético'/'Diabético'."""
    return 'No Diabético' if pred_num == 0 else 'Diabético'
...
def predecir():
    try:
        ...
        proba    = modelo.predict_proba(X_sc)[0].tolist()  # [p_no_diabetico, p_diabetico]

        return jsonify({
            'prediccion'  : decodificar(pred_num),
            'clase'       : pred_num,          # 0=No Diabético, 1=Diabético
            'probabilidad': {
                'No Diabético': round(proba[0] * 100, 2),
                'Diabético': round(proba[1] * 100, 2)
            },
...
# El CSV debe tener las 8 columnas de features + columna "Outcome" (opcional)
# ════════════════════════════════════════════════════════════
...
def predecir_lotes():
    try:
        ...
        tiene_etiquetas = 'Outcome' in df.columns
        ...
        for i, (pred, proba) in enumerate(zip(preds, probas)):
            resultados.append({
                ...
                'clase'           : int(pred),
                'prob_no_diabetico': round(float(proba[0]) * 100, 2),
                'prob_diabetico'  : round(float(proba[1]) * 100, 2)
            })
        ...
        respuesta = {
            ...
            'resumen': {
                'No Diabético': int(np.sum(preds == 0)),
                'Diabético'   : int(np.sum(preds == 1))
            }
        }

        # Si el CSV tiene etiquetas reales → calcular métricas
        if tiene_etiquetas:
            y_real = df['Outcome'].values.astype(int)
